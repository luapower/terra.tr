...
local HB_LANGUAGE_EN = hb_language_from_string('en', 2)
local HB_LANGUAGE_DE = hb_language_from_string('de', 2)
local HB_LANGUAGE_ES = hb_language_from_string('es', 2)
local HB_LANGUAGE_FR = hb_language_from_string('fr', 2)
local HB_LANGUAGE_RU = hb_language_from_string('ru', 2)
local HB_LANGUAGE_ZH = hb_language_from_string('zh', 2)

local terra ub_lang(hb_lang: hb_language_t): cstring
...
terra TextRenderer:shape(text_runs: &TextRuns, segments: &Segs)
			...
			if i == len or (i > 0 and str[i-1] == PS) then
				...
				if par_len > 0 then
					...
					fribidi_get_bidi_types(
						str + par_offset,
						par_len,
						self.bidi_types.elements + par_offset
					)
	...
	for i, len, lang in self:lang_runs(len) do
		set_linebreaks_utf32(str + i, len + 1, ub_lang(lang), self.linebreaks:at(i))
	end
	...
	if text_runs.runs.len > 0 then
		...
		for i = 0, len+1 do --NOTE: going one char beyond the text!
			...
			var text_run1, font1, font_size1, features1
			...
			var level1, script1, lang1
			if len == 0 then

				--the string is empty so init those with defaults.
				var dir = (text_run1.dir or 'auto'):lower()
			...
			var linebreak_code = i > 0 and linebreaks[i-1] or 2
